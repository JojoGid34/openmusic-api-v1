...
class PlaylistsService {
  constructor(collaborationsService, playlistActivitiesService) {
    ...
    this._playlistActivitiesService = playlistActivitiesService; // Digunakan untuk mencatat aktivitas
  }
  ...
  async addSongToPlaylist(playlistId, songId, owner) {
    ...
    await this._playlistActivitiesService.addActivity({
      playlistId,
      songId,
      userId: owner, // You need to get the user ID here
      action: 'add',
    });
  }
  ...
  async getSongsInPlaylist(playlistId) {
    const query = {
      text: `SELECT playlists."playlistId", playlists.name, users.username,
             CASE
               WHEN COUNT(songs."songId") > 0 THEN json_agg(json_build_object('id', songs."songId", 'title', songs.title, 'performer', songs.performer))
               ELSE '[]'::json
             END AS songs
             FROM playlists
    ...
    // This is the mapping change you need to make:
    return {
